...
#define RDMA_DEFAULT_CQ_ENTRIES 32 /* Number of completion queue entries */

/* Queue Pair Configuration */
#define RDMA_DEFAULT_QP_SEND_WR 16 /* Maximum outstanding send work requests \
                                    */
#define RDMA_DEFAULT_QP_RECV_WR \
  16 /* Maximum outstanding receive work requests */
#define RDMA_DEFAULT_QP_SGE_COUNT \
  1 /* Number of scatter-gather elements per WR */
#define RDMA_DEFAULT_MTU IBV_MTU_1024 /* Maximum transmission unit */
...
#define RDMA_DEFAULT_RNR_RETRY \
  7 /* Number of RNR (Receiver Not Ready) retries */
#define RDMA_DEFAULT_MIN_RNR_TIMER 12 /* Minimum RNR NAK timer */
#define RDMA_DEFAULT_MAX_RD_ATOMIC \
  4 /* Maximum outstanding RDMA read/atomic ops */

/*
 * Buffer Management
 * Memory allocation and data transfer configuration
 */
#define PG_BUFFER_SIZE_BYTES \
  (1024 * 1024)                  /* 1MB buffer for collective operations */
