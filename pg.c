...
static int establish_neighbor_connections(pg_handle_internal_t *process_group) {
  ...
  if (left_tcp_socket >= 0) {
    ...
    printf("[Process %d] DEBUG: Sending right QP num to left neighbor: %u\n", 
           process_group->process_rank, right_local_info.queue_pair_number);

    if (pgnet_exchange_rdma_bootstrap_info(left_tcp_socket, &right_local_info,
                                           &left_remote_info,
  ...
  if (right_tcp_socket >= 0) {
    ...
    printf("[Process %d] DEBUG: Sending left QP num to right neighbor: %u\n", 
           process_group->process_rank, left_local_info.queue_pair_number);

    if (pgnet_exchange_rdma_bootstrap_info(right_tcp_socket, &left_local_info,
                                           &right_remote_info,
...
int pg_test_rdma_connectivity(pg_handle_t process_group_handle) {
  ...
  /* Check if memory regions are properly registered */
  if (!process_group->left_send_mr || !process_group->left_receive_mr ||
      !process_group->right_send_mr || !process_group->right_receive_mr) {
    fprintf(stderr, "[Process %d] Memory regions not properly registered\n", 
            process_group->process_rank);
    return PG_ERROR;
  }
